...
def load_settings() -> dict:
    if not os.path.exists("settings.json5"):
        raise FileNotFoundError(
            "settings.json5 not found. Copy settings.example.json5 to settings.json5 and configure it."
        )
    ...
    for url in config["feeds"]:
        ...
        for entry in feed.entries:
            ...
            if announce_type == "new" or announce_type == "cross":
                desc = normalize_text(
                    entry["description"].split("Abstract:")[-1].strip(), L2T
                )
                authors = [
                    normalize_text(author.strip(), L2T)
                    for author in entry.get("author", "").split(",")
                ]
                entries.append(
                    Entry(
                        normalize_text(entry["title"].strip(), L2T),
                        entry["link"].strip(),
                        desc,
                        authors,
                    )
...
def create_preference_vector(config, model: SentenceTransformer) -> np.ndarray:
    vecs = []
    w = config.get(
        "weights", {"keywords": 10, "liked_texts": 10, "trusted_authors": 30}
    )
    ...
    if config.get("liked_texts"):
        liked_vecs = model.encode(
            config.get("liked_texts", []), normalize_embeddings=True
        )
        vecs.append(
            (np.mean(liked_vecs, axis=0), w.get("liked_texts", 1.0) / len(liked_vecs))
        )
...
def rank_papers(
    config, model: SentenceTransformer, entries: list[Entry], vec: np.ndarray
...
def write_results(
    ...
    pubdate: time.struct_time,
    now: datetime.datetime,
):
    now_str = now.strftime("%Y-%m-%d %H:%M:%S")
    pubdate_str1 = time.strftime(
        "%Y-%m-%d %H:%M:%S", time.localtime(time.mktime(pubdate) + 9 * 3600)
    )
